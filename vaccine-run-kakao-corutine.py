...
#ToDO: open login page and get new cookie
...
async def find_vaccine(vaccine_type, top_x, top_y, bottom_x, bottom_y):
    ...
    while not done:
        try:
            ...
            try:
                response = requests.post(url, data=json.dumps(data), headers=Headers.headers_map, verify=False, timeout=3)
                json_data = json.loads(response.text).get("organizations")
            except requests.exceptions.Timeout:
                print("병원 조회가 원할하지 않습니다. 재 검색하겠습니다.")
                continue
            end_time = time.time()

            print(datetime.now())
            print("조회 병원 수 : %d " % len(json_data), "/ 검색시간 : %s 초" % round((end_time-start_time),3))
